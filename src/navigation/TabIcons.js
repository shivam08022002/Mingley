...
// Tab 1 — Discover (two stacked cards)
export const DiscoverTabIcon = React.memo(({ focused }) => {
  const { theme } = useTheme();
  const fill = focused ? theme.iconActive : theme.iconInactive;
  return (
    <View style={styles.wrap}>
      {focused && <View style={[styles.bar, { backgroundColor: theme.iconActive }]} />}
      <Svg width={ICON_W} height={ICON_H} viewBox={VIEWBOX} fill="none">
        {/* Right card (upright) */}
        <Rect
          x="27.499" y="16.4976" width="13" height="18" rx="2"
          fill={fill} stroke={theme.navBackground} strokeWidth="0.8"
        />
  ...
  return (
    <View style={styles.wrap}>
      ...
      <Svg width={ICON_W} height={ICON_H} viewBox={VIEWBOX} fill="none">
        ...
        <Circle
          cx="38" cy="18" r="5"
          fill={focused ? theme.iconActive : theme.iconInactive}
          stroke={theme.navBackground} strokeWidth="2"
        />
      </Svg>
    </View>
  );
